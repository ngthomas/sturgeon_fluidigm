...
# set path
#setwd("data/assess_5thChip_rr/raw")
...
sturgeon.file <- lapply(chip, function(x) {
  read.csv(paste("data/assess_5thChip_rr/raw/",x, "_raw.csv", sep =""), 
           skip = 15,
...
# Function: creates a new log log-likelihood scale transformation object
...
```{r}
data.join <- join(data.reduce, rbind(region.unk.result,
                                     n.region.lou,
                                     s.region.lou) , by = "name", type="inner")

ggplot(data.join %>%
         filter(total.k > 0 )%>%
         group_by(name) %>%
         summarise(n_unassign_cluster=sum(new.k==0), prob.N = prob.being.north[1]),
       aes(x=n_unassign_cluster,y = prob.N))+
  geom_rect(xmax=Inf, xmin=-Inf, ymin=-0.99, ymax=0.99, fill="pink", alpha=0.02)+
  geom_point()+
  xlab("Number of unassigneable loci (out of 74)")+
  ylab("Prob of being in North DPS")+
  theme_bw()+
  #annotation_logticks(base = 10, sides="l")+
  scale_y_continuous(trans="log_split", breaks = c(1e-16, 1e-6, 0.01, 0.1, 0.5, 0.9, 0.99, 1-(1e-6), 1-(1e-16)),
...
data.join.unassign <- data.join %>%
         filter(total.k > 0 )%>%
         group_by(name) %>%
         summarise(n_unassign_cluster=sum(new.k==0), prob.being.north = prob.being.north[1], region.lab=region)


p<- ggplot()+
  geom_rect(data=rbind(n.region.lou, s.region.lou, region.unk.result), aes(xmax=Inf, xmin=-Inf, ymin=0.01, ymax=0.99,alpha=0.002), fill="light pink")+
  geom_point(data=data.join.unassign %>%
               filter(re), aes(x=as.numeric(as.factor(name)), y=prob.being.north, color=n_unassign_cluster))+
  geom_point(data=s.region.lou, aes(x=as.numeric(as.factor(name)), y=prob.being.north, color=n_unassign_cluster))+
  geom_point(data=region.unk.result, aes(x=as.numeric(as.factor(name)), y=prob.being.north, color=n_unassign_cluster))+
  xlab("Individual Sample")+
  ylab("Prob of being in North DPS")+
  theme_bw()+
  #annotation_logticks(base = 10, sides="l")+
