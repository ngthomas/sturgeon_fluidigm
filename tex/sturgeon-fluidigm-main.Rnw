...
three types. The duplicate DNA samples are unambiguously apparent from their high $\Lambda_{ij}$ values, and,
clearly, none of the {\bf NKS} pairs are samples from the same fish.
\label{fig:self-id}}
\end{figure*}
...
which falls between the previously recognized ranges of the two DPSs, were unambiguously 
from the Northern DPS. These fish were apparently reproducing (J. Strange \& S. Kullman, pers. comm.), 
which, if accurate, indicates a range expansion for the species. Finally, we show how these 
...
and are considerably less expensive and less labor-intensive to genotype and analyze. 
...
originated in the Rogue River, the only other basin with documented green sturgeon 
