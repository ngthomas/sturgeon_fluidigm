...
these SNP assays, 413 (corresponding to \Sexpr{N_for_structure} distinct green sturgeon, since some
...
The \Sexpr{N_for_structure} individuals with 60 or more called loci were included in the clustering analysis with {\em structure}. 
...
\caption{\footnotesize Relationship between number of loci score and the posterior probabilities of self-assignments 
...
\end{center}
\caption{\footnotesize Location and DPS-origin of fishery bycatch samples. Left panel is a coastwide
...
%% Down at the end, we use the .tex file to typeset a two-column version
%% for submission to ConsGen
<<typeset-cons-gen, include=FALSE, cache=FALSE, eval=FALSE>>=
library(stringr)
library(dplyr)
tex <- readLines("tex/sturgeon-fluidigm-main.tex")
abs <- tex[which(str_detect(string = tex, pattern = "begin\\{abstract\\}")) :
       which(str_detect(string = tex, pattern = "end\\{abstract\\}"))]
main <- tex[which(str_detect(string = tex, pattern = "START MAIN BODY")) :
       which(str_detect(string = tex, pattern = "END MAIN BODY"))] %>%
  str_replace_all(., "section\\*", "section") %>%
  str_replace_all(., "\\.\\./outputs", "inputs") %>%
  str_replace_all(., "\\.\\./StructureArea/clump_and_distruct/final_pdf", "inputs") %>%
  str_replace_all(., "\\\\footnotesize", "") %>%
  str_replace(., "section\\{Acknowledgments\\}", "begin{acknowledgements}")

main[length(main) - 1] <- "\\end{acknowledgements}"

  # write it out to a tmp file then catenate and latex it
  cat(c(abs, main), file = "tex/cons-gen/cons-gen-body.tex", sep = "\n")
  
  # then cp all the files we would like contained within the cons-gen directory
  system("cp outputs/sample_summary.tex outputs/chip_location_counts.tex outputs/cross_chip_shared_samples.tex outputs/four_loci_by_plate-crop.pdf outputs/four_loci_by_geno-crop.pdf outputs/successful-assay-histogram-crop.pdf StructureArea/clump_and_distruct/final_pdf/BB_ds_Clumped_TopLabel_k002r001.pdf StructureArea/clump_and_distruct/final_pdf/BB_ds_Clumped_NoLabel_k003r001.pdf StructureArea/clump_and_distruct/final_pdf/BB_ds_Clumped_NoLabel_k004r001.pdf outputs/self-ass-plot-crop.pdf outputs/bycatch_map-crop.pdf outputs/lambda_densities-crop.pdf outputs/obs-self-id-logls-crop.pdf tex/sturgeon-fluidigm.bib tex/cons-gen/inputs/ ")
  
  # then typeset it
  system("cd tex/cons-gen; cat cons-gen-pream.tex cons-gen-body.tex cons-gen-postam.tex > cons-gen-sturgeon.tex; latexmk -pdf cons-gen-sturgeon.tex")
  
  # then transfer the supplemental figures and the table over to a directory to be latexed together
  system("cd outputs; source ~/.bashrc;  cp plate_x_y_with_num_clusts_*.pdf plate_x_y_by_final_genotype_plates_combined*.pdf ../tex/suppl_forge/images/")
  
  # then latex them
  system("source ~/.bashrc;  cd tex/suppl_forge; pdflatex suppl1.tex && cp suppl1.pdf ../suppl_final/suppl1-snp-scatter-figures.pdf")
  
  # finally do the table
  file.copy(from = "outputs/supp-tab-1-green-sturgeon-snps.csv", to = "tex/suppl_final/suppl2-green-sturgeon-snps-table.csv", overwrite = TRUE)
@
