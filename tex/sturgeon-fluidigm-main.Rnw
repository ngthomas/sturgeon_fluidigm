...
bycatch, and five adult fish sampled from the Eel River, CA.   See Table~\ref{tab:samps} for a summary
of sample sizes.
\begin{table*}
...
\label{tab:samps}
\end{table*}
Tissue samples were predominantly fin clips dried or stored in ethanol, apart from the eggs which were whole developing eggs, stored in ethanol, and in the case of the Klamath River reference samples, whole, young juveniles in ethanol. 
...
Upper Sacramento River. 21.24 million sequence reads of
XXXX base pairs generated by the sequencing run were assembled into 84,747 putatively unique genetic 
loci (regions of the genome) and, within those regions, we identified XXX,XXX SNPs
using Stacks software v 1.12 \citep{catchen2011stacks} with the 
following parameter settings ($m = XX$, $M = XX$, XXXX). At 1,118 of the SNPs the four Klamath
...
small number of samples, and in our opinion, is a preferable strategy to using the program
...
approach, rather than using the underlying modeling framework of {\sc FitTetra}.
...
not well resolved in fluorescent intensity.  This approach allows the user to define genotypes
...
We first used four chips (our {\em scoring-development chips}) to decide which loci were consistently scorable, and at each, the number of genotype clusters
that could be resolved.  Some individuals were genotyped on multiple chips which allowed us to visually account for 
chip-to-chip variation (Table~\ref{tab:devo-samps}).  
\begin{table}
...
Fluidigm chip reader.  The {\em raw} intensities are those tht have not been transformed based on the
observed intensity of the two no-template controls (NTCs) on the chip.
Raw intensities are not available from the
Fluidigm XXXX Software by default, but, with a request and a code from
Fluidigm technical support (ph.~XXX-XXX-XXXX), they can be
accessed.  We used the raw intensities at this stage because there was less chip-to-chip variability in them
compared to the standard, NTC-normalized values available from the software. On these plots we colored the
results from each chip a different color and connected the same sample genotyped on different chips with
line segments.  An example for four loci with 2, 3, 4, and 5 clusters regarded as scorable, respectively,
...
shows the results at four plates for one locus.  The numerial in the upper right gives the number
of genotype categories that are scored at the locus.  Each point represents an individual's fluorescence
intensities at two dyes. (a) Inidividuals are colored according to plate and dashed line
segments connect the intensities of individuals typed on different chips. (b) Individuals are colored
according to the genotype category they have been scored as.  Category~0 (colored light gray) is a ``no call'', \ie the individual
