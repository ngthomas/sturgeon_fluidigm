...
\begin{tabular}{llllr}
\hline \hline \\ 
\input{inputs/sample_summary.tex}
\hline \hline \\ 
\end{tabular}
\end{center}
\label{tab:samps}
...
combinations of 96 assays and 96 individuals on a single array (hereafter called ``chip''Äù). 
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.87\textwidth]{inputs/flow-chart-crop.pdf}
\end{center}
\caption{Graphic depiction of workflows.  SNP Discovery done only once.  Development
of SNP calling methodology need be undertaken only once.  Then the steps for 
...
\begin{center}
\begin{tabular}{lrrr}
\input{inputs/chip_location_counts.tex}
\hline
\end{tabular}
\end{center}
\mbox{}\\
...
\begin{center}
\begin{tabular}{lrrrr}
\input{inputs/cross_chip_shared_samples.tex}
\hline
\end{tabular}
\end{center}
\hrule
...
Our reference samples did not include any known spawners from the 
Rogue River---an additional
population in the Northern DPS.  However, a previous microsatellite study
...
of genotypes to develop the chip scoring methodology, occurs when
...
be appropriate to consider recalibrating the scoring methodology as new samples come in.
...
has previously provided quite a good degree of resolution between
the fish from the two DPSs \citep{Israeletal2009}. However, these SNP assays provide substantially 
better resolution than the microsatellites (see \textcolor{red}{Supporting Information XX}) 
