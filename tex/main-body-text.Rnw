%!TEX root = sturgeon-fluidigm-main.Rnw
...
We received samples of two general categories.  Samples collected in freshwater rivers known to have well-established spawning populations of green sturgeon are ``reference'' samples. Some of these samples were taken from juvenile fish that must have been spawned in the river from which they were sampled.  Others were taken from adult fish that may be non-spawning visitors that are actually native to a river different from the one they were sampled in.  We refer to these groups as juvenile reference and adult reference samples, respectively.  Samples taken from the ocean or from rivers without an historically abundant spawning population are ``non-reference`` samples.  They are comprised of green sturgeon caught as bycatch, and three juvenile??????? fish sampled from the Eel River, CA.  Summaries of these samples appear in Table~\ref{tab:samps}
\begin{table*}
\caption{Summary of genetic samples used in this study.}
\begin{tabular}{lrrrrrrrr}
\hline \hline \\ 
\input{../outputs/bycatch_table.tex}
\hline \hline \\ 
\end{tabular}
