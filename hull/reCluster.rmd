...
#Take one North sample out and re-calculate its predictive label
reduce.data %>%
...
# Calculate the predictive probabilities for BC labels
...
for ( set in 1:ncol(comb.set) ){
  ...
  melt.NS <- melt.share.k.r %>% 
    ...
    mutate(compare.reg = ifelse(reg.1!=reg.2,"North_South",reg.1)) 
...
multiplot(plotlist=plot.by.region, cols=3)
