...
p.val.KS <- apply(cbind(north.obs, south.obs) , 1 , function(x) {
    num.k <- as.numeric(x[16])
    ks.test(rep(1:num.k, as.numeric(x[3:(3+num.k-1)])),
                    rep(1:num.k, as.numeric(x[11:(11+num.k-1)])))$p.val
    })
...
grp.frac.plot <- ggplot(rbind(melt.table.frac.N, melt.table.frac.S), aes(x=assay.name,y=fraction, fill=variable))+
  facet_wrap(~region)+
...
ggsave("DPS-discriminant-loci.pdf",grp.frac.plot, height=20, width=30)
```
...
ggsave("PCA1_2_defined_k.pdf", height=20, width=30)
```
